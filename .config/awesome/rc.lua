...
-- This determines which layout windows are created in by default
...
local theClock = wibox.widget.textclock('%H:%M\n%d')

local theCalendar = awful.widget.calendar_popup.year()
theCalendar:attach(theClock, "tr")
...
globalkeys = gears.table.join(
    awful.key({ modkey,           }, "p",      hotkeysPopup.show_help,
    ...
    awful.key({ modkey, "Shift" }, "t", awesome.quit,
...
for i = 1, 11 do
    ...
    globalkeys = gears.table.join(globalkeys,
        ...
        awful.key({ modkey, "Control", "Shift" }, kN,
                  function ()
                      if client.focus then
                          local tag = client.focus.screen.tags[i]
                          if tag then
...
-- Enable focus on hover, so that focus follows mouse.
